...
class MemberProperty
{
    ...
    public function getDividend($id) {
        $dividend_rate = Dividend::where('rate_year', Diamond::today()->year)->first();
        $forward_amount = Shareholding::where('member_id', $id)->whereYear('pay_date', '<', Diamond::today()->year)->sum('amount');
        $forward = collect([(object)[
            'name' => 'ยอดยกมา', 
            'shareholding' => $forward_amount / 10, 
            'amount' => $forward_amount, 
            'dividend' => (is_null($dividend_rate) ? 0 : $forward_amount * ($dividend_rate->rate / 100) * (Diamond::today()->month / Diamond::today()->month)),
            'remark' => (is_null($dividend_rate) ? 'ยังไม้ได้กำหนดอัตราเงินปันผล' : '')
        ]]);
        $dividends = $forward->merge(Shareholding::where('member_id', $id)
            ->whereYear('pay_date', '=', Diamond::today()->year)
            ->select(DB::raw('str_to_date(concat(\'1/\', month(pay_date), \'/\', year(pay_date)), \'%d/%m/%Y\') as name'),
                ...
                DB::raw(is_null($dividend_rate) ? 
                    '0 as dividend' : 
                    '(sum(amount) * ' . ($dividend_rate->rate / 100) . ' * (' . Diamond::today()->month . ' - month(pay_date)) / ' . Diamond::today()->month . ') as dividend'),
