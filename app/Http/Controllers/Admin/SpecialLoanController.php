...
class SpecialLoanController extends Controller
{
    ...
    public function getCreateEmployeeLoan($member_id, Request $request) {
        $loan = is_null(Loan::find($request->input('loan_id'))) 
            ? LoanManager::create_loan($member_id, $request->input('loantype_id')) 
            : Loan::find($request->input('loan_id'));
        $member = Member::find($member_id);

        switch (intval($request->input('step'))) {
            default:
                return view('admin.loan.create.special.employee.create', [
                    'step' => 1,
                    'loan' => $loan
                ]);
                break;
    ...
    protected function validateEmployeeStep1(Request $request) {
        ...
        $validator->after(function($validator) use ($request, $loan) {
            ...
            // ตรวจสอบเงินเดือนผู้กู้ (วงเงินที่ต้องการกู้ต้องไม่เกิน 3 เท่าของเงินเดือนและสูงสุดไม่เกิน 150,000 บาท)
    ...
    public function getCreateOutsiderLoan($member_id, Request $request) {
        ...
        switch (intval($request->input('step'))) {
            default:
                return view('admin.loan.create.special.outsider.create', [
                    'step' => 1,
                    'loan' => $loan
                ]);
                break;
