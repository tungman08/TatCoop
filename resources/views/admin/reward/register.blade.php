    ...
    <section class="content">
        ...
        <div class="well">
            ...
            <table>
                <tr>
                    <td>
                        {{ Form::open(['action' => ['Admin\RewardController@postCloseRegister', $reward->id], 'method' => 'post', 'onsubmit' => "return confirm('คุณต้องการปิดการลงทะเบียน เพื่อเริ่มจับรางวัลใช่ไหม?');"]) }}
                            {{ Form::button('<i class="fa fa-times"></i> ปิดการลงทะเบียน', [
                                'type' => 'submit', 
                                'disabled' => !(($is_super || $is_admin) && $reward->rewardStatus->id < 3),
                                'class'=>'btn btn-danger btn-flat'])
                            }}
                        {{ Form::close() }}  
                    </td>
                    <td class="padding-l-xl">
                        @if (($is_super || $is_admin) && $reward->reward_status_id == 3)
                            <a href="{{ action('Admin\RewardController@getLate', ['id' => $reward->id]) }}">ลงทะเบียนล่าช้า</a>
                        @endif
                    </td>
                </tr>
            </table>
